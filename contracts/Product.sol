...
contract ProductList {
    ...
    function addProduct(string memory _name, uint256 _price) public {
        ...
        products[productCount] = Product(productCount + 1, _name, _price); // Add the product at the last index
        productCount++; // Increment the product counter
    }

    // Function to get product details by index
    function getProduct(uint256 _index) public view returns (uint256, string memory, uint256) {
    ...
    function deleteProduct(uint256 _index) public {
        ...
        products[productCount - 1] = Product(0, "", 0);
    ...
    function searchProduct(string memory _name) public view returns (uint256, uint256) {
        for (uint256 i = 0; i < productCount; i++) {
            if (keccak256(abi.encodePacked(products[i].name)) == keccak256(abi.encodePacked(_name))) {
                return (products[i].id, products[i].price); // Return ID and price if found
            }
        }
